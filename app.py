      ...
      <div class="hero-content">
        ...
        <div class="login-card">
          ...
          <div class="storage-notice">
            <strong>Ver.3.0</strong> - クラウド対応版：データはサーバーに安全に保存されます
          </div>
        </div>
      </div>
    </section>
        ...
        <div class="testimonials-grid">
          <div class="testimonial-card">
            <div class="testimonial-content">
              ...
              <p class="testimonial-text">
                「備品の所在がすぐに分かるようになり、探す時間が大幅に短縮されました。職員の負担が軽減され、より利用者様のケアに集中できています。」
              </p>
            </div>
            <div class="testimonial-author">
              <div class="author-info">
                <h4>田中 美穂 様</h4>
                <span>特別養護老人ホーム さくら園</span>
                <span class="role">看護師長</span>
              </div>
            </div>
          </div>
          ...
          <div class="testimonial-card">
            <div class="testimonial-content">
              <div class="quote-icon">✨</div>
              <p class="testimonial-text">
                「備品の貸出・返却の記録が自動化されたことで、管理業務が格段に楽になりました。月末の備品チェックも短時間で完了できるようになりました。」
              </p>
            </div>
            <div class="testimonial-author">
              <div class="author-info">
          ...
          <div class="testimonial-card">
            <div class="testimonial-content">
              <div class="quote-icon">💖</div>
              <p class="testimonial-text">
                「備品の在庫状況がリアルタイムで把握できるため、購入計画も立てやすくなりました。無駄な購入を防げて、コスト削減にも貢献しています。」
              </p>
            </div>
            <div class="testimonial-author">
              <div class="author-info">
                <h4>高橋 恵子 様</h4>
                <span>特別養護老人ホーム 青空の家</span>
                <span class="role">総務課長</span>
              </div>
            </div>
          </div>
          ...
          <div class="testimonial-card">
            <div class="testimonial-content">
              <div class="quote-icon">🌟</div>
              <p class="testimonial-text">
                「システムが直感的で使いやすく、ITが苦手な職員でもすぐに操作を覚えられました。導入から1週間で全員が使いこなしています。」
              </p>
            </div>
            <div class="testimonial-author">
              <div class="author-info">
                <h4>伊藤 正夫 様</h4>
                <span>有料老人ホーム 陽だまりテラス</span>
                <span class="role">ケアマネージャー</span>
              </div>
            </div>
          </div>
        </div>
  ...
  <div id="mainContent" class="main-content" style="display: none;">
    <div class="compact-header">
      <div class="logo-section">
        <div>
          <h1>StockEasy</h1>
          <p class="sub-heading">らくらく備品管理システム</p>
        </div>
      </div>
  ...
  <script>
    ...
    async function apiCall(url, options = {}) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error('APIエラー詳細:', errorText); // デバッグ用
        ...
        tr.innerHTML = `
          <td>
            <div class="image-wrapper">
              ...
              ${isAdmin ? `<button class="edit-button" onclick="editImage(${index})">画像</button>` : ''}
            </div>
          </td>
          <td>${item.name}</td>
          <td>${item.id}</td>
          ...
          <td><input type="text" value="${item.note || ''}" onchange="updateNote(${index}, this.value)" /></td>
          <td>${item.category}</td>
          <td>
            <button onclick="openBorrow(${index})">借用</button>
            <button onclick="openReturn(${index})">返却</button>
            ${isAdmin ? `<button onclick="openEdit(${index})">編集</button>` : ''}
          </td>
        `;
    ...
    async function confirmEdit() {
      ...
      try {
        ...
        await apiCall(`${API_BASE}/equipment/${oldItem.id}`, {
          method: 'PUT',
          body: JSON.stringify({
            name,
            location,
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('roleSelect').addEventListener('change', function() {
        ...
        if (this.value === 'admin') {
          ...
          document.getElementById('adminPassword').placeholder = '管理者パスワード';
        } else {
